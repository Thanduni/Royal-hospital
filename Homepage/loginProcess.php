...
if (isset($_POST['login'])) {
    if (empty($_POST["email"]) || empty($_POST["password"])) {
        ...
        $query = $con->prepare("select nic,name, email, password, user_role, profile_image, otp, verify from user where email=?;");
        $query->bind_param("s", $_POST["email"]);
        $query->execute();
        $query->store_result();
        $query->bind_result($nic, $name, $email, $password, $userRole, $profilePic, $otp, $verify);
