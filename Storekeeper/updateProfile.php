...
if (isset($_POST['updateStorekeeper'])) {
    echo "<pre>";
    ...
    if(($email != $_SESSION['mailaddress'] && $row[0]>0))
        header("location:" . BASEURL . "/Storekeeper/updateStorekeeperProfile.php?wrongResult=The email address already exists!");
    ...
    if ($result)
        header("location:" . BASEURL . "/Storekeeper/updateStorekeeperProfile.php?correctResult=The receptionist profile is updated successfully!");
}
...
if (isset($_POST['changePassword'])){
    ...
    if(!password_verify($_POST['oldPassword'], $row[0])){
        header("location:" . BASEURL . "/Storekeeper/updateStProfile.php?wrongResult=The old password you have entered is wrong!");
    }
    else {
        if ($_POST['newPassword'] != $_POST['confirmPassword'])
            header("location:" . BASEURL . "/Storekeeper/updateStorekeeperProfile.php?wrongResult=The passwords you have entered is different!");
        else{
            $hash = password_hash($_POST['newPassword'], PASSWORD_DEFAULT);
            $query = "UPDATE user SET  password='$hash' WHERE email = '" . $_SESSION['mailaddress'] . "'";
            $result = mysqli_query($con, $query);
            if($result)
                header("location:" . BASEURL . "/Storekeeper/updateStorekeeperProfile.php?correctResult=The password is changed successfully!");
            else
                die("fail");
        }
    }
